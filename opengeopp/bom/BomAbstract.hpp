#ifndef __OPENGEOPP_BOM_BOMABSTRACT_HPP
#define __OPENGEOPP_BOM_BOMABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const OPENGEOPP::BomAbstract& iBom) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            OPENGEOPP::BomAbstract& ioBom) {
  // Fill Bom object with input stream
  ioBom.fromStream (ioIn);
  return ioIn;
}

#endif // __OPENGEOPP_BOM_BOMABSTRACT_HPP
