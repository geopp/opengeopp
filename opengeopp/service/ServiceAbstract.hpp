#ifndef __OPENGEOPP_SERVICEABSTRACT_HPP
#define __OPENGEOPP_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const OPENGEOPP::ServiceAbstract& iService) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            OPENGEOPP::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __OPENGEOPP_SERVICEABSTRACT_HPP
