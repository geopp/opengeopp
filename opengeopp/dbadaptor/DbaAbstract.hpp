#ifndef __OPENGEOPP_COM_DBA_DBAABSTRACT_HPP
#define __OPENGEOPP_COM_DBA_DBAABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const OPENGEOPP::DbaAbstract& iDba) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            OPENGEOPP::DbaAbstract& ioDba) {
  // Fill Dba object with input stream
  ioDba.fromStream (ioIn);
  return ioIn;
}

#endif // __OPENGEOPP_COM_DBA_DBAABSTRACT_HPP
